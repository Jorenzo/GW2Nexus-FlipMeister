...
CompletedTrackerModule::CompletedTrackerModule(EntryData* entry)
{
  Entry = entry;
  ReadFromFile();
}
...
void CompletedTrackerModule::ReadFromFile()
{
  ...
  if (!inFile) {
    Log(Entry, WARNING, "Failed to open completed tracker file at '%s'", path.c_str());
    return;
  }
  ...
  try
  {
    ...
    Log(Entry, WARNING, "Failed to parse JSON in completed tracker file '%s'\n %s", path.c_str(), e.what());
    return;
  }
  ...
  try
  {
    ...
    Log(Entry, WARNING, "JSON structure mismatch in completed tracker file '%s'\n %s", path.c_str(), e.what());
    return;
  }
}
...
void CompletedTrackerModule::WriteToFile()
{
  ...
  if (!std::filesystem::exists(dirPath)) {
    if (!std::filesystem::create_directories(dirPath)) {
      Log(Entry, WARNING, "Failed to write completed tracker file at '%s'", path.c_str());
    }
  }
  ...
  try {
    ...
    Log(Entry, WARNING, "Failed to write completed tracker at file '%s'\n%s", path.c_str(), e.what());
  }
}